...
export const TEE_NAMED_EXPORTS: Array<{ name: string; detail: string }> = [
  ...
  { name: "router", detail: "router(self, { routes, mode? }) — writes $route / $router. Hash default." },
  { name: "Link", detail: "Link — <Link to=\"/about\">. Uses $router.push." },
  { name: "RouterView", detail: "RouterView — outlet. Same route pattern reuses the page instance." },
