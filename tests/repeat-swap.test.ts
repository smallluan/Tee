...
describe("keyed t-repeat", () => {
  ...
  it("disposes detached fast row owners with their containing structure", async () => {
    const { app, host } = mount({
      template:
        `<ul t-show="visible">` +
    ...
    expect(app.maps().reverse.filter((site) => site.label === "repeat row bindings")).toHaveLength(2);
